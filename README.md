...
| – | Planner ▸ Preset | `Default (claude)` | Runs the planner through one of the agent presets, so planning uses the same binary, wrapper and environment as the work — pick the sandboxed preset and planning happens inside the sandbox. The preset's command gets `-p --model <model>` and the prompt appended, runs in the project directory, and has its `$VAR`s expanded: there is no task yet, so `HARMONIUM_TASK_GIT_ROOT` and `HARMONIUM_TASK_WORKDIR` are both the project and `HARMONIUM_TASK_BRANCH` is unset. Settings-only; a planner command overrides it. |
